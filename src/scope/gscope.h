...
public:
  shared_ptr<linked_hashmap<string, FuncType>> funcs;
  shared_ptr<linked_hashmap<string, ClassInfo>> classes;
  virtual ValueType GetIdentifierType(const string &identifier) const override;
  GScope();
  virtual ValueType
  GetClassMember(const string &class_identifier,
                 const string &member_identifier) const override;
  virtual void CheckIdentifier(const string &identifier)
      const override; // ensure the var or func identifier not used before as a
                      // class identifier.
