...
ConstructFuncDefNode::ConstructFuncDefNode(
    MxParser::ConstructFuncDefContext *ctx) {
  func_identifier = ctx->Identifier()->getText();
  body = make_shared<StmtBlockNode>(ctx->stmtBlock());
}
...
NewExprNode::NewExprNode(MxParser::NewExprContext *ctx) {
  if (auto now_node = ctx->newObjExpr()) {
    ...
    type = ObjectType(now_node->basicType()->getText(),
                      now_node->LeftBracket().size() +
                          now_node->arrayIndex().size()); // todo
  } else
    throw MyException("Unknown error in AST building");
}
...
ExprStmtNode::ExprStmtNode(MxParser::ExprStmtContext *ctx) {
  expr = make_shared<ExpressionNode>(ctx->expression());
}
} // namespace AST
